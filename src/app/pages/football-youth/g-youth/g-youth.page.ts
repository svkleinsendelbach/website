...
import { TextSectionComponent, EventsComponent, ReportsComponent, ContactsComponent, MapsComponent, ContactsData, EventGroup, ReportGroup, Result, FirebaseApiService, ResultDisplayComponent } from 'kleinsendelbach-website-library';
...
@Component({
    ...
    standalone: true,
    imports: [CommonModule, TextSectionComponent, EventsComponent, ReportsComponent, ContactsComponent, MapsComponent, ResultDisplayComponent],
...
export class GYouthPage {
    ...
    private async fetchEventGroups() {
        this.eventGroupsResult = await this.firebaseApi.function('event-get').call({
            groupIds: ['football-youth/g-youth']
        });
    }
