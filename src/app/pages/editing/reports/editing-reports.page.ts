...
export class EditingReportsPage {
    ...
    public reportGroups: ReportGroup.Flatten[] | undefined = undefined;
    ...
    public getReportGroupOf(groupId: ReportGroupId): ReportGroup.Flatten | undefined {
        return this.reportGroups?.find(reportGroup => reportGroup.groupId === groupId);
    }

    public async deleteReport(groupId: ReportGroupId, reportId: string) {
        this.reportGroups = this.reportGroups?.flatMap(reportGroup => {
            if (reportGroup.groupId !== groupId)
                return reportGroup;
            const reports = reportGroup.reports.filter(report => report.id !== reportId);
            if (reports.length === 0)
                return [];
            return {
    ...
    public async editReport(groupId: ReportGroupId, report: Report.Flatten) {
